   ...
   
{
    ...
    memset(fDeviceID,0,sizeof(fDeviceID));
	 memset(fStopFlag,0,sizeof(fStopFlag));
    fSetDeviceID =0;
    res_data_node = NULL;
}
...
ReflectorSession::~ReflectorSession()
{
    ...
    qtss_printf("Removing ReflectorSession: %x,DeviceID:%s,StopFlag:%s\n", this,fDeviceID,fStopFlag);
    // For each stream, check to see if the ReflectorStream should be deleted
    if(fDeviceID != NULL || NULL != fStopFlag)
    {
        this->ClearLiveInfo(fDeviceID);        
        QTSServerPrefs* inPrefs= QTSServerInterface::GetServer()->GetPrefs();
        int NumBalance = inPrefs->GetNumValues(qtssPrefsDataBaseUrl);
        if(stopStream(sPHInterface,fDeviceID,fStopFlag)<=0)
        {
            qtss_printf("stopStream failed\n");
        }else
        {
            res_data_t *node = (res_data_t *)res_data_node;
            if(node)
            {
                node ->statu =0;
                node->try_time  =0;
    ...
    if(sStatisticsHost)
    {      
        sendCameraPerson(sStatisticsHost,this, 0);        
    }  
