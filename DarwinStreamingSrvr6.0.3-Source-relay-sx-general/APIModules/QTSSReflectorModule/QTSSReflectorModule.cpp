...
static QTSS_AttributeID         sResDataAttr                =   qtssIllegalAttrID;
...
static UInt32  sARTSNumUpLeverServer;
//static QTSS_ServerObject sServer = NULL;
QTSS_ModulePrefsObject       sPrefs = NULL;
res_data_t * res_data_list = NULL;
res_data_t * last_res_data = NULL;
static OSMutex*        sRESDATDMutex;
//
// Prefs
...
static char*  sARTSUpLeverServer[MAX_HANDLE_DIR] = {0,};


int ARTS_MODULE_DEBUG_LEVEL =1;
PH_Interface *sPHInterface = NULL;
/**
 * global var
 */
Bool16 g_iswasu = false;
Bool16 g_isjinshan = false;
Bool16 g_isningxia = false;
char *g_StbUserAgent = NULL;
//static char g_StbUserAgent[512] = {'\0'};

Bool16 sDefaultwasu = false;
Bool16 sDefaultjinshan = false;
Bool16 sDefaultningxia = false;
//sip项目需求 //for sip add/zlj
static char *   sDataBaseHost=NULL;
char *   sStatisticsHost= NULL;
...
static void GetLocalHost( char *host);

// add/zlj  0830
char* GetUrlForStop(QTSS_StandardRTSP_Params* inParams);
...
QTSS_Error Register(QTSS_Register_Params* inParams)
{
	...
	static char*        sResDataObjName                 = "QTSSResDataObj";
	...
	(void)QTSS_AddStaticAttribute(qtssClientSessionObjectType, sResDataObjName, NULL, qtssAttrDataTypeVoidPointer);
	(void)QTSS_IDForAttr(qtssClientSessionObjectType, sResDataObjName, &sResDataAttr);
	...
	sRESDATDMutex = NEW OSMutex();
	sPHInterface = NEW PH_Interface();
...
QTSS_Error GetUpLevelServer()
{
	QTSS_Object theAttrInfo = NULL;
	QTSS_Error theErr = QTSS_GetAttrInfoByName(sPrefs, "up_level_server", &theAttrInfo);
	...
	QTSS_GetNumValues(sPrefs, theID, &sARTSNumUpLeverServer);
	Assert(theErr == QTSS_NoErr);    

	if (sARTSNumUpLeverServer == 0)
		return NULL;

	for (UInt32 theIndex = 0; theIndex < sARTSNumUpLeverServer; theIndex++)
	{
		theErr = QTSS_NoErr;
		(void)QTSS_GetValueAsString((QTSS_Object)sPrefs, theID, theIndex, &sARTSUpLeverServer[theIndex]);
		Assert(theErr == QTSS_NoErr);   
	}
	return QTSS_NoErr;
}
...
QTSS_Error RereadPrefs()
{
	...
	//zlj add 0806
	QTSSModuleUtils::GetAttribute(sPrefs, "arts_iswasu", 	qtssAttrDataTypeBool16,
			&g_iswasu, &sDefaultwasu, sizeof(g_iswasu));
	QTSSModuleUtils::GetAttribute(sPrefs, "arts_isjinshan", 	qtssAttrDataTypeBool16,
			&g_isjinshan, &sDefaultjinshan, sizeof(g_isjinshan));
	QTSSModuleUtils::GetAttribute(sPrefs, "arts_isningxia", 	qtssAttrDataTypeBool16,
			&g_isningxia, &sDefaultningxia, sizeof(g_isningxia));
	g_StbUserAgent = QTSSModuleUtils::GetStringAttribute(sPrefs, "stb_useragent", "Defalut");
	...
	sDataBaseHost = QTSSModuleUtils::GetStringAttribute(sPrefs, "couchbase_host", sBindLocalHost);

	delete []sStatisticsHost;
	sStatisticsHost = QTSSModuleUtils::GetStringAttribute(sPrefs, "statistics_host", sBindLocalHost);
...
QTSS_Error ReflectorSession :: ClearLiveInfo(char *vid)
{
	if (vid == NULL || sDataBaseHost == NULL || strlen(sDataBaseHost) ==0 )  return -1;
	memcached_st * memc = NULL;
	char key[1024]={'\0'};
	...
	if(memc == NULL)
	{
		RLogRequest(INFO_ARTS_MODULE,0,"connect %s failed",sDataBaseHost);
		return QTSS_RequestFailed;
	}

	sprintf(key,"%s_src_host",vid);   
	int rc = mem_set(memc,key,NULL,86400); 

	if(rc == MEMCACHED_SUCCESS)
	{
		RLogRequest(INFO_ARTS_MODULE,0,"clear %s success",vid);
	}else
		RLogRequest(INFO_ARTS_MODULE,0,"clear %s failed",vid);
...
QTSS_Error SaveLiveInfo(char *src_host,UInt32  src_port,char *vid)
{
	if (!vid) return -1;
	memcached_st * memc = NULL;
	char key[1024]={'\0'};
	...
	sprintf(key,"%s_src_host",vid);
	sprintf(addr,"%s:%d",src_host,src_port);
	int rc = mem_set(memc,key,addr,86400); 

	if(rc == MEMCACHED_SUCCESS)
	{
		RLogRequest(INFO_ARTS_MODULE,0,"set %s success",vid);
	}else
		RLogRequest(INFO_ARTS_MODULE,0,"set %s failed",vid);

	release_conn(memc);
	return QTSS_NoErr;
}

void call_host_port(char *ip, int port , char * DeviceID,char *sdp_buf,char*vid,UInt32 l_callid,QTSS_StandardRTSP_Params* inParams)
{
	...
	RLogRequest(INFO_ARTS_MODULE,0,"ip:%s,port:%d,id:%s",ip,port,DeviceID);

	if(ip && strlen(ip) && port >0 && DeviceID&& strlen(DeviceID) && sDataBaseHost != NULL && strlen(sDataBaseHost)>0 )
	{
		//SaveLiveInfo(ip,port,DeviceID);
	}
	...
	if(Stb_User_Agent(UserAgentStr.Ptr) == TCP_MODULE)
	{
		sprintf(sdp_buf+strlen(sdp_buf),"m=video %d MP2T TCP\r\n\r\n",port);
	}

	if(Stb_User_Agent(UserAgentStr.Ptr) == UDP_MODULE)
	{
		sprintf(sdp_buf+strlen(sdp_buf),"m=video %d udp MP2T\r\n\r\n",port);
	}

	if(Stb_User_Agent(UserAgentStr.Ptr) == WUXI_MODULE)
	{
		sprintf(sdp_buf+strlen(sdp_buf),"m=video %d MP2T mpgv\r\na=control:trackID=1\r\n\r\n",port);
	}
	if(Stb_User_Agent(UserAgentStr.Ptr) == AVIT_MODULE)                                                                                                                                                                       
	{
...
int get_validate_host_port(char *Bhost,char *Bport,CCommnication *obj,int l_callid)
{
	QTSServerPrefs* inPrefs= QTSServerInterface::GetServer()->GetPrefs();
	int NumBalance = inPrefs->GetNumValues(qtssPrefsDataBaseUrl);
	...
	for (int i=0;i<NumBalance;i++)
	{
		...
		inPrefs->GetValueAsString(qtssPrefsDataBaseUrl, i, &theIPAddrStr);
		RLogRequest(DEBUG_ARTS_MODULE,l_callid,"theIPAddrStr:%s",theIPAddrStr);
		if(obj!=NULL)
		{
			RLogRequest(DEBUG_ARTS_MODULE,l_callid,"obj ip:%s",obj->m_szLiveIp);
		}
		if(theIPAddrStr != NULL && strlen(theIPAddrStr) )
		{
			if(obj!= NULL && strstr(theIPAddrStr,obj->m_szLiveIp) != NULL)
			{
				continue;
			}


			char *port = strstr(theIPAddrStr,":");
			if(port)
			{
				memcpy(Bhost,theIPAddrStr,port - theIPAddrStr);

				port += 1;
				memcpy(Bport,port,strlen(port));
				RLogRequest(DEBUG_ARTS_MODULE,l_callid,"host:%s,port:%s\n",Bhost,Bport);
				break;
			}
		}
	}
...
CCommnication *  reconnect_balance(CCommnication * old_obj,int l_callid)
{
	char Bhost[128]={'\0'};
	char Bport [128]={'\0'};
	int ret =0;

	get_validate_host_port(Bhost,Bport,old_obj,l_callid);
	if(strlen(Bhost) ==0 || strlen(Bport) ==0)
	{
		RLogRequest(INFO_ARTS_MODULE,l_callid,"get no host and port");
		return NULL;
	}

	CCommnication *cur_obj = new CCommnication();   

	cur_obj->init(Bhost,Bport,NULL,0,NULL,0);
	RLogRequest(INFO_ARTS_MODULE,l_callid,"recon to %s:%s",Bhost,Bport);
	if(  (ret= cur_obj->CreateConnect() ) < 0)
	{
		RLogRequest(INFO_ARTS_MODULE,l_callid,"connect %s failed\n",Bhost);
	}else               
		if( (ret =cur_obj->Register(0)) == FALSE)
		{
			RLogRequest(INFO_ARTS_MODULE,l_callid,"Register failed");
		}

	if( (ret = cur_obj->SendPort(NULL,0,NULL)) == FALSE)
	{
		RLogRequest(INFO_ARTS_MODULE,l_callid,"send port failed");
	}
...
QTSS_Error  generate_sdp(char *vid,UInt32 l_callid,char *sdp_buf,Bool16 isCamera,char *local_host,char*src_host,char *url,char*user,char*pwd,int module,res_data_t **node,QTSS_StandardRTSP_Params* inParams)
{
	...
	{
		UInt32 src_port = 0;
		GetSrcHost(src_host,&src_port,vid,l_callid);
	}else
	{

		p = get_node(res_data_list,url,NULL);
		if(p ==  NULL)
		{
			RLogRequest(INFO_ARTS_MODULE,l_callid,"new res_data_list and insert res_data_list");
			p = (res_data_t*)malloc(sizeof(res_data_t));
			memset( p,0,sizeof(res_data_t));
			if(url&& strlen(url))
			{
				strcpy(p->url,url);
			}    
			...
			}
			{
				...
				if(url!=NULL && strlen(url) >0 && (strstr(url,"NVR") || strstr(url,"nvr")))
				{
					insert_node(&res_data_list,&last_res_data,p);
				}     
		...
		RLogRequest(INFO_ARTS_MODULE,l_callid,"**sendStream");
		sendStream(sPHInterface,url,user,pwd,vid,module);
		p->statu = SENDSTREAMRESP;
		(*node) = p;

		/*
		   CCommnication  * obj =  g_balance_conn;

		//connect_commnication();
		bool err = FALSE;
		int r_try =1;
retry:
if(obj)
{      
err = obj->SendStream(vid,url,user,pwd,module);
RLogRequest(INFO_ARTS_MODULE,l_callid,"SendStream ret:%d",err);

if(err == FALSE)
{    
while(r_try < NumBalance)
{        
g_balance_conn = reconnect_balance(obj,l_callid);
if(g_balance_conn)
break;
else
r_try ++;          
...
if( obj->GetStream(&call_host_port,sdp_buf,vid) != 0)
{
return QTSS_RequestFailed;
}
}*/
...
void DoDescribeAddRequiredSDPLines(QTSS_StandardRTSP_Params* inParams, ReflectorSession* theSession, QTSS_TimeVal modDate,  ResizeableStringFormatter *editedSDP, StrPtrLen* theSDPPtr)
{
	...
	if (!checkedSDPContainer.HasReqLines())
	{
		...
		{ // add s line
			...
			//editedSDP->Put(theSDPName); //zlj/add 0928
			editedSDP->PutEOL();
		}
		...
		{ // add o line
			...
			editedSDP->PutEOL();
			/*
			   editedSDP->Put("a=range:npt=0-36000.00 ");
			   editedSDP->PutEOL();*/
		}
	} 
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params* inParams)
{
	...
	if (theSession == NULL){
		//RLogRequest(INFO_ARTS_MODULE, l_callid,"zlj DoDescribe setup ReflectorSession faild!.........应该加入send balance stop");
		//char DeviceID[256] = {0};
		//strcpy(DeviceID,GetUrlForStop(inParams));
		//if (NULL == DeviceID){
		//	return QTSS_RequestFailed;
		//}
		//stopStream(sPHInterface,GetUrlForStop(inParams));//add/zlj 0830
...
/*
   CCommnication * connect_commnication()
   {
   char* theIPAddrStr = NULL;
   QTSServerPrefs* inPrefs= QTSServerInterface::GetServer()->GetPrefs();
   QTSS_Error  theErr = inPrefs->GetValueAsString(qtssPrefsDataBaseUrl, 0, &theIPAddrStr);

   if (theErr != QTSS_NoErr)
   {
   qtss_printf("liveEncode list host port is not configure\n");
   }
   CCommnication *obj = NULL;
   char host[32]={'\0'};
   char *port = NULL;
   if( ( port = strstr(theIPAddrStr,":") )  != NULL )
   {

   memcpy(host,theIPAddrStr,port - theIPAddrStr);

   port += 1;
   qtss_printf("host:%s,port:%s\n",host,port);

   obj = new CCommnication();
   obj->init(host,port,NULL,0,NULL);

   if( obj->CreateConnect() != 0)
   {
...
void findStr(char *src_buf,char*dst_buf,char*str)
{
	if(!src_buf || !dst_buf || !str)
	{
		return;
	}
	char temp_buf[256]={'\0'};
	char *p = strstr(src_buf,str);
	char *end =  NULL;
	char *real_end = NULL;
	int real_len =0;
	if(p)
	{
		...
		if(p)
		{
			...
			if(end)
			{   memcpy(temp_buf,p,end-p);             

			}else 
				strncpy(temp_buf,p,strlen(p));
		}       
		...
		if(strlen(temp_buf))
		{
			...
			if(end)
			{
				real_len = end-temp_buf;
			}else
				real_len = strlen(temp_buf);

			memcpy(dst_buf,temp_buf,real_len);
		}
	}
}


void GetParamFromQry(char *qurl,char*src_url,char*user_name,char*pwd,char *moduleStr)
{
	if(qurl && strlen(qurl) <0)
		return;

	findStr(qurl,src_url,"url=");
	findStr(qurl,user_name,"user=");
	findStr(qurl,pwd,"pwd=");
	findStr(qurl,moduleStr,"module=");
}


void  sendCameraPerson(char *host,ReflectorSession* theSession,int l_callid)
{
	if(theSession == NULL || host == NULL)
	{
		return ;
	}
	CCommnication *cur_obj = new CCommnication();   
	...
	if(p)
	{
		memcpy(Bhost,host,p-host);
		p++;
		memcpy(Bport,p,strlen(p));  
		...
		{
			RLogRequest(INFO_ARTS_MODULE,l_callid,"connect %s failed\n",Bhost);
		}else if(cur_obj->SendPersonNum(theSession->GetRef()->GetRefCount(),theSession->GetDeviceID()) <0)
		{
			RLogRequest(INFO_ARTS_MODULE,l_callid,"SendPersonNum failed\n");
		} 

		RLogRequest(INFO_ARTS_MODULE,l_callid,"send Person :%d,DeviceID:%s",theSession->GetRef()->GetRefCount(),theSession->GetDeviceID());                     
	}        
	delete cur_obj;
}

char* GetUrlForStop(QTSS_StandardRTSP_Params* inParams){	
	UInt32      eURLSize = 4096;
	char urlQryBuf[4096] = { 0 };
	char url[4096] = { 0 };
	QTSS_ClientSessionObject     theClientSession   = inParams->inClientSession;
	StrPtrLen urlQry(urlQryBuf, eURLSize -1);
	(void)QTSS_GetValue(theClientSession, qtssCliSesReqQueryString, 0, urlQry.Ptr, &urlQry.Len);
	findStr(urlQryBuf,url,"url=");
	res_data_t *p = NULL;
	p = get_node(res_data_list,url,NULL);
	if (p == NULL){
		return NULL;
...
{   
	...
	char pwd[256]={'\0'};
	char modulestr[256]={'\0'};
	char buf[32] = {'\0'};
	UInt32 cLength = 0;
	int module = 0;
	QTSS_Error err ;
	res_data_t *node = NULL;
	res_data_t **pnode = NULL;
	err = QTSS_GetValuePtr(inParams->inClientSession,sResDataAttr,0,(void**)&pnode,&l_len);
	if(pnode)
	{
		node= (*pnode);
	}
	qtss_printf("node:%x,pnode:%x\n",node,pnode);
	...
	err = QTSS_GetValue(theClientSession, sARTSClientSessionAttr, 0, (void *)&l_callid, &l_len);

	StrPtrLen urlQry(urlQryBuf, eURLSize -1);
	(void)QTSS_GetValue(theClientSession, qtssCliSesReqQueryString, 0, urlQry.Ptr, &urlQry.Len);

	GetParamFromQry(urlQryBuf,src_url,user_name,pwd,modulestr);
	RLogRequest(DEBUG_ARTS_MODULE,l_callid,"urlQryBuf:%s,src_url:%s,user_name:%s,pwd:%s",urlQryBuf,src_url,user_name,pwd);
	GetLocalHost(local_host); 
	if(strlen(modulestr) >0)
		module = atoi(modulestr);
	...
	if (theSessionRef == NULL)
	{
		...
		{   
			...
			if(node== NULL)
			{
				generate_sdp(vid,l_callid,sdp_buf,isCamera,local_host,src_host,src_url,user_name,pwd,module,&node,inParams);
				if(node)
				{                    
					(void)QTSS_SetValue(inParams->inClientSession, sResDataAttr, 0, &node, sizeof(node));
				}

				qtss_printf("node 1:%x\n",node);
			}

			qtss_printf("zlj--node->statu:%d,strlen(node->ip): %d,strlen(node->DeviceID):%d,node->port: %d\n",node->statu,strlen(node->ip),strlen(node->DeviceID),node->port);

			if(node->statu !=RECVSTREAMRESP || strlen(node->ip) == 0 || strlen(node->DeviceID)==0 || node->port ==0)
			{
				if(node->idelTime >= MAXIDLETIME)
				{
					RLogRequest(INFO_ARTS_MODULE,l_callid,"idelTime over");
					if(node->try_time >= g_balanceNum)//DISCONN->2
					{
						RLogRequest(INFO_ARTS_MODULE,l_callid,"get sendStream failed");
						RLogRequest(INFO_ARTS_MODULE,l_callid,"zlj get sendStreamfailed  node->statu:%d,node->try_tim:%d,node->idelTime:%d\n",node->statu,node->try_time,node->idelTime);
					...
					{                        
						res_data_t * node1= NULL;  
						(void)QTSS_SetValue(inParams->inClientSession, sResDataAttr, 0, &node1, sizeof(node1));
						node->idelTime = 0;
						node->try_time ++;
						RLogRequest(INFO_ARTS_MODULE,l_callid,"try send again,try_time:%d,g_balanceNum:%d,node:%x",node->try_time,g_balanceNum,node);
					}                                     
				}

				RLogRequest(INFO_ARTS_MODULE,l_callid,"node->statu:%d,strlen(node->ip):%d,strlen(node->DeviceID):%d,node->port:%d",
						node->statu,strlen(node->ip),strlen(node->DeviceID),node->port);
				RLogRequest(INFO_ARTS_MODULE,l_callid,"fresh_server:%d,conn_statu:%d,time:%"_64BITARG_"d",sPHInterface->fresh_server,sPHInterface->conn_statu,QTSS_Milliseconds());

				//when server is reconnect ,will try send again ,no until timeout   
				/*
				   if(sPHInterface->fresh_server== true)
				   {
				   node->idelTime = 0;
				   }

				   if(sPHInterface->fresh_server == true && sPHInterface->conn_statu == 1)
				   {
				   res_data_t * node1= NULL;  
				   (void)QTSS_SetValue(inParams->inClientSession, sResDataAttr, 0, &node1, sizeof(node1));
				   sPHInterface->fresh_server = false;
				   RLogRequest(INFO_ARTS_MODULE,l_callid,"try send again 2");
				   }*/
			...
			call_host_port(node->ip, node->port , node->DeviceID,sdp_buf,vid,l_callid,inParams);
			//zlj  add length
			StrPtrLen sdpData_t(sdp_buf);                                                                                                                                                                      
			char buf[32];
			cLength = sdpData_t.Len-29;
			qtss_sprintf(buf, "%"_S32BITARG_"", sdpData_t.Len-29);
			//(void)QTSS_AppendRTSPHeader(inParams->inRTSPRequest, qtssContentLengthHeader,&buf[0], ::strlen(&buf[0]));

			if(strlen(sdp_buf) ==0)
			{
				RLogRequest(DEBUG_ARTS_MODULE,l_callid,"generate_sdp failed!\n");
				return NULL;
			}else
			{
		...
		theSession->SetTotalLength(cLength); //zlj add for nds
		...
		if(theSession->res_data_node == NULL)
		{
			theSession->res_data_node = node;
			qtss_printf("theSession:%x,node:%x\n",theSession,node);
		}
	...
	{   
		...
		RLogRequest(INFO_ARTS_MODULE,l_callid,"reflectorSess_ref:%x,ref_count:%d,theSession:%x",theSession->GetRef(),theSession->GetRef()->GetRefCount(),theSession);  
		if(theSession->fSetDeviceID == FALSE)
		{  
			...
			if (strstr(src_url,"windows:") != NULL){
				...
				theSession->SetStopFlag("stop8200");		
			}else if (strstr(src_url,"sip:") != NULL){
				theSession->SetStopFlag("stopsip");		
			}else{
				theSession->SetStopFlag("stoprtsp");	
			}
			theSession->fSetDeviceID = TRUE;
		}
		char *id = theSession->GetDeviceID();
		char *stopflag = theSession->GetStopFlag();
		RLogRequest(INFO_ARTS_MODULE,l_callid,"DeviceID:%s,StopFlag:%s",id,stopflag);        
	}
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params* inParams)
{
	...
	if(sStatisticsHost)
	{      
		sendCameraPerson(sStatisticsHost,theSession, l_callid);        
	}
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params* inParams, ReflectorSession* inSession)
{
	...
	{  
		...
		if (sPlayResponseRangeHeader)
		{
			...
			if (theErr != QTSS_NoErr)
			{
				//StrPtrLen range("npt=0-36000.00");
				...
				//StrPtrLen info("0.000");
				//QTSS_AppendRTSPHeader(inParams->inRTSPRequest,qtssScaleHeader,scale.Ptr,scale.Len);b
			}
		}
