...
class ReflectorSession
{
    public:
        ...
        void    SetDeviceID(char *pDeviceID) {strcpy(fDeviceID,pDeviceID);}
		char *    GetStopFlag() {return  fStopFlag;}
		void    SetStopFlag(char *pStopFlag) {strcpy(fStopFlag,pStopFlag);}
		
		UInt32    GetTotalLength() {return  fTotalLength;}
		void    SetTotalLength(UInt32 plength) {fTotalLength = plength;}
        QTSS_Error ClearLiveInfo(char *vid);
        //
        // ACCESSORS
...
extern CCommnication *g_balance_conn ;
extern char *   sStatisticsHost;
void  sendCameraPerson(char *host,ReflectorSession* theSession,int l_callid);
CCommnication * connect_commnication();
int get_validate_host_port(char *Bhost,char *Bport,CCommnication *obj,int callid);
CCommnication *  reconnect_balance(CCommnication * old_obj,int callid);
#endif
