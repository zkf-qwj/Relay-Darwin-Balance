...
void call_host_port(char *ip, int port , char * DeviceID,char *sdp_buf,char*vid)
{
...
QTSS_Error  generate_sdp(char *vid,UInt32 l_callid,char *sdp_buf,Bool16 isCamera,char *local_host,char*src_host,char *url,char*user,char*pwd,int module,res_data_t **node)
{
...
{   
	...
	if (theSessionRef == NULL)
	{
		...
		{   
			...
			if(node== NULL)
			{
				generate_sdp(vid,l_callid,sdp_buf,isCamera,local_host,src_host,src_url,user_name,pwd,module,&node);
				if(node)
				{                    
			...
			call_host_port(node->ip, node->port , node->DeviceID,sdp_buf,vid);
