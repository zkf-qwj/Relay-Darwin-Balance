...
static Bool16  SetUpUDPSocketsPool(QTSServerPrefs* inPrefs)
{
	...
	UInt32 NumBalance =  inPrefs->GetNumValues(qtssPrefsDataBaseUrl);
	g_balanceNum = NumBalance;
	int seqNumber = inPrefs->GetSeqNumber();
	    qtss_printf("theNumPorts:%d,theMinPort:%d,seqNumber:%d,NumBalance:%d\n",theNumPortsTotal,theMinPort,seqNumber,NumBalance);
	...
	theErr = inPrefs->GetValueAsString(qtssPrefsBindUDPPortsIPAddr, 0, &theBindIPAddr);
	 int *ports = (int *)malloc(sizeof(int) * theNumPortsTotal);
		...
		if( AddrStrPtr && (  ::strcmp(AddrStrPtr->Ptr, "127.0.0.1") != 0)  &&  theNumPortsTotal >0 && theMinPort >0  && theMinPort > kLowestUDPPort && theMinPort < kHighestUDPPort)
		{
			...
			for(UInt32 theNumPorts =0; theNumPorts < theNumPortsTotal; theNumPorts ++)
			{
				...
				if (thePair != NULL)
				{
					...
					ports[theNumAllocatedPairs] = theMinPort + 2*theNumPorts;
	...
	{ 
		...
		{   char host[32]={'\0'};
			...
			g_balanceHost =(char**)malloc(sizeof(char*) *theNumAllocatedPairs );


			for(i=0;i<NumBalance;i++)
			{
				inPrefs->GetValueAsString(qtssPrefsDataBaseUrl, i, &theIPAddrStr);
				char *temp= (char *)malloc(strlen(theIPAddrStr)+1);
				strcpy(temp,theIPAddrStr);
				g_balanceHost[i]= temp;
				qtss_printf("theIPAddrStr:%s\n",theIPAddrStr);
			}
			i=0;
			{  
				...
				if( ( port = strstr(theIPAddrStr,":") )  != NULL )
				{
					...
					sPHInterface->Configure(host,port,theNumAllocatedPairs,ports,theBindIPAddr,seqNumber);                 
...
{
	RLogRequest(DEBUG_ARTS_MODULE,0,"1.Entring ReflectorSender::ReflectPackets");
...
SInt64 ReflectorSocket::Run()
{
	...
	{             
		...
		if (theSender2 != NULL )
		{
			...
			{
				RLogRequest(DEBUG_ARTS_MODULE,0,"run---->ReflectPackets\n");
...
{   
	...
	if (thePacket != NULL) do
	{
		...
		RLogRequest(DEBUG_ARTS_MODULE,0,"1.GetLocalPort():%d",GetLocalPort());
		...
		{
			RLogRequest(DEBUG_ARTS_MODULE,0,"2.thePacket->fPacketPtr.Len:%d",thePacket->fPacketPtr.Len);
		...
		if (thePacket->IsRTCP())
		{
			RLogRequest(DEBUG_ARTS_MODULE,0,"3.thePacket->IsRTCP:%d",thePacket->IsRTCP());
		...
		if (0) //turn on / off buffer size checking --  pref can go here if we find we need to adjust this
			...
			{
				RLogRequest(DEBUG_ARTS_MODULE,0,"4.........");
...
void ReflectorSocket::GetIncomingData(const SInt64& inMilliseconds)
{
	...
	RLogRequest(DEBUG_ARTS_MODULE,0,"1.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort); 
	...
	RLogRequest(DEBUG_ARTS_MODULE,0,"2.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort);
	...
	while (true)
	{
		...
		RLogRequest(DEBUG_ARTS_MODULE,0,"3.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort);
		...
		RLogRequest(DEBUG_ARTS_MODULE,0,"4.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort);
		if(sPacketDataInHeaderBufferLen ==0)
		{
			fRTSPClient = NULL;
		}
		...
		if(  this->GetTransportType() == qtssRTPTransportTypeTCP && this->fTCPDataSocket)
		{
			...
			RLogRequest(DEBUG_ARTS_MODULE,0,"4.1.this->fTCPDataSocket->Read:theRecvLen:%d",theRecvLen);
			RLogRequest(DEBUG_ARTS_MODULE,0,"4.2.kMaxReceivePacketSize-fReceiveBufPtr.Len:%d",kMaxReceivePacketSize-fReceiveBufPtr.Len);

			//qtss_printf("rece_packet_len:%d\n",theRecvLen);                  
		}
		RLogRequest(DEBUG_ARTS_MODULE,0,"5.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort);
		...
		if(this->GetTransportType() == qtssRTPTransportTypeUDP)
		{
			...
			RLogRequest(DEBUG_ARTS_MODULE,0,"6.thePacket->fPacketPtr.Len:%d",thePacket->fPacketPtr.Len); 
		}
...
handle_tcp:  
		...
		{
			this->GetMediaData(thePacket);  //将tcp接收到的数据拷贝到thePacket->fPacketPtr.Ptr里面以供ReflectPackets-->SendPacketsToOutput-->WritePacket 
		}   

		RLogRequest(DEBUG_ARTS_MODULE,0,"7.theRemoteAddr:%d,theRemotePort:%d",theRemoteAddr,theRemotePort);
